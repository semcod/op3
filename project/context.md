...
### src.opstree.cli.main.scan
> Scan a device and output snapshot.
- **Calls**: cli.command, click.argument, click.option, click.option, click.option, click.option, LayerTree, ProbeRegistry
...
### src.opstree.cli.main.drift
> Detect drift between intended and actual state.
- **Calls**: cli.command, click.argument, click.argument, Path, Path, Snapshot.load, DriftDetector, detector.detect

### src.opstree.cli.main.convert
> Convert between configuration formats.
- **Calls**: cli.command, click.argument, click.argument, click.option, Path, None.write_text, click.echo, LessAdapter

### src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe._scan_drm
> Skanuj wyjścia DRM.
- **Calls**: ctx.execute, hasattr, None.splitlines, hasattr, hasattr, re.match, m.group, ctx.execute
...
- **Calls**: ctx.execute, hasattr, None.splitlines, hasattr, ctx.execute, ctx.execute, _get_int, _get_int
...
> Zeskanuj warstwę.
- **Calls**: ProbeResult, self._probe_board_model, self._scan_drm, self._scan_backlights, self._check_kms, self._get_kms_driver, LayerData, datetime.now
...
### src.opstree.probes.builtin.runtime_container.RuntimeContainerProbe.anomalies
> Wykryj anomalie w kontenerach.
- **Calls**: data.data.get, anomalies.append, container.get, container.get, container.get, container.get

### src.opstree.probes.builtin.business_health.BusinessHealthProbe.anomalies
> Wykryj anomalie w zdrowiu aplikacji.
- **Calls**: data.data.get, data.data.get, anomalies.append, anomalies.append, data.data.get, data.data.get

### src.opstree.probes.context.SSHContext.__init__
> Args:
    target: SSH target in format "user@host"
    metadata: Additional metadata
    ssh_key_path: Path to SSH private key (optional)
- **Calls**: None.__init__, ssh_cmd.extend, subprocess.run, ExecuteResult, ssh_cmd.extend, super

### src.opstree.layers.tree.LayerDefinition.to_fraq_node
> Konwertuj na FraqNode dla użycia przez fraq.core.
- **Calls**: len, FraqNode, self.id.split, hash, float, float

### src.opstree.probes.builtin.os_linux.OsConfigProbe.scan
> Zeskanuj warstwę.
- **Calls**: self._read_config_txt, self._read_cmdline, ProbeResult, LayerData, datetime.now

### src.opstree.probes.builtin.runtime_container.RuntimeContainerProbe.scan
> Zeskanuj warstwę.
- **Calls**: self._detect_runtime, self._list_containers, ProbeResult, LayerData, datetime.now
...
### Flow 5: drift
```
drift [src.opstree.cli.main]
```

### Flow 6: convert
```
convert [src.opstree.cli.main]
```
...
### Flow 8: _scan_backlights
```
_scan_backlights [src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe]
```

### Flow 9: _detect_runtime
```
...
### Flow 10: _get_uptime
```
_get_uptime [src.opstree.probes.builtin.os_linux.OsKernelProbe]
```
...
- **Methods**: 8
- **Key Methods**: src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe.can_probe, src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe.scan, src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe._probe_board_model, src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe._scan_drm, src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe._scan_backlights, src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe._check_kms, src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe._get_kms_driver, src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe.anomalies
...
- **Methods**: 3
- **Key Methods**: src.opstree.probes.registry.ProbeRegistry.register, src.opstree.probes.registry.ProbeRegistry.get, src.opstree.probes.registry.ProbeRegistry.all
...
### src.opstree.probes.context.ExecuteResult
> Result of command execution.
- **Methods**: 1
- **Key Methods**: src.opstree.probes.context.ExecuteResult.ok

### src.opstree.probes.context.ProbeContext
> Base context for probe execution.
- **Methods**: 1
- **Key Methods**: src.opstree.probes.context.ProbeContext.__init__

### src.opstree.probes.context.LocalContext
> Local execution context (runs commands on localhost).
- **Methods**: 1
- **Key Methods**: src.opstree.probes.context.LocalContext.__init__
- **Inherits**: ProbeContext
...
### src.opstree.cli.main.convert
> Convert between configuration formats.
- **Output to**: cli.command, click.argument, click.argument, click.option, Path
...
- `src.opstree.cli.main.scan` - 39 calls
- `src.opstree.formats.less.LessAdapter.render` - 38 calls
- `src.opstree.formats.less.LessAdapter.parse` - 37 calls
- `src.opstree.cli.main.drift` - 26 calls
- `src.opstree.cli.main.convert` - 26 calls
...
- `src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe.scan` - 8 calls
...
- `src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe.anomalies` - 4 calls
...
- `src.opstree.probes.registry.register_probe` - 2 calls
...
- `src.opstree.probes.builtin.endpoint_http.EndpointHttpProbe.can_probe` - 2 calls
- `src.opstree.probes.builtin.physical_rpi.RpiPhysicalDisplayProbe.can_probe` - 2 calls
- `src.opstree.snapshot.model.Snapshot.query` - 2 calls
- `src.opstree.snapshot.model.Snapshot.to_yaml` - 2 calls
...
graph TD
    ...
    _scan_backlights --> execute
    _scan_backlights --> hasattr
    _scan_backlights --> splitlines
    _detect_runtime --> execute
    _detect_runtime --> hasattr
```
