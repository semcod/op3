...
class EndpointHttpProbe:
    ...
    def _check_endpoint(self, ctx: ProbeContext, url: str, method: str) -> dict:
        ...
        stdout, rc = _exec(f"curl -s -o /dev/null -w '%{{http_code}}' -X {method} {url} 2>/dev/null")
        ...
        stdout_time, rc_time = _exec(f"curl -s -o /dev/null -w '%{{time_total}}' -X {method} {url} 2>/dev/null")
        try:
            response_time_ms = int(float(stdout_time.strip()) * 1000) if rc_time == 0 else None
