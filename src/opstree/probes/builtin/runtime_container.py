...
class RuntimeContainerProbe:
    ...
    def _list_containers(self, ctx: ProbeContext, runtime: str) -> list[dict]:
                ...
                try:
                    ...
                    for c in container_list:
                        containers.append({
                            ...
                            "state": c.get("State", ""),
                            "status": c.get("Status", ""),
                            "restart_policy": c.get("Labels", {}).get("io.podman.annotations.restartpolicy", ""),
                        })
                    return containers
                ...
                try:
                    ...
                    for c in container_list:
                        containers.append({
                            ...
                            "state": c.get("State", ""),
                            "status": c.get("Status", ""),
                            "restart_policy": c.get("Labels", {}).get("com.docker.compose.restart", ""),
                        })
                    return containers
                except json.JSONDecodeError:
                    pass
        ...
        cmd = f"{runtime} ps -a" if runtime != "unknown" else "docker ps -a 2>/dev/null || podman ps -a 2>/dev/null"
        stdout, rc = _exec(cmd)
        if rc == 0:
    ...
    def anomalies(self, data: LayerData) -> list:
        ...
        for container in containers:
            if container.get("state") == "exited" and container.get("restart_policy") in ["always", "unless-stopped"]:
                anomalies.append({
                    "severity": "warning",
                    "layer": self.layer_id,
